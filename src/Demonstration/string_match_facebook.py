...
def Coordinates1(new_dict):
    return Coordinates(latitude = checkNestedLookup('latitude', new_dict),
                        longitude = checkNestedLookup('longitude', new_dict))


def PhotoMetadata1(new_dict):
    return PhotoMetadata(upload_ip = checkNestedLookup('upload_ip', new_dict))
...
def Media1(new_dict):
    return Media(uri = checkNestedLookup('uri', new_dict),
                title = checkNestedLookup('title', new_dict),
                description = checkNestedLookup('description', new_dict),
                creation_timestamp = checkNestedLookup('creation_timestamp', new_dict),
                media_metadata = MediaMetadata1(new_dict))



def Place1(new_dict):
    return Place(name = checkNestedLookup('name', new_dict),
...
def ExternalContext1(new_dict):
    return ExternalContext(url = checkNestedLookup('url', new_dict))

def Data1(new_dict):
    return Data(post = checkNestedLookup('post', new_dict),
                title = checkNestedLookup('title', new_dict),
                update_timestamp = checkNestedLookup('update_timestamp', new_dict),
                timestamp = checkNestedLookup('timestamp', new_dict),
...
def Post1(new_dict):
    return(Post(title = checkNestedLookup('title', new_dict),
                timestamp = checkNestedLookup('timestamp', new_dict),
                data = Data1(new_dict),
                attachments = Attachments1(new_dict) ,
                tags = checkNestedLookup('tags', new_dict)))
